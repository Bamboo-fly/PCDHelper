...
class MainWindow(QMainWindow):
    ...
    def load_pcd_file(self):
        ...
        if file_path:
            try:
                ...
                if new_pcd is not None and len(new_pcd.points) > 0:  # 检查点云数据是否有效
                    self.vis.clear_geometries()
                    ...
                    # 计算每个点到参考点的距离
                    distances = np.linalg.norm(np.asarray(new_pcd.points) - reference_point, axis=1)

                    # 将距离映射到颜色（例如，较远的点为蓝色，较近的点为红色）
                    max_distance = np.max(distances)
                    min_distance = np.min(distances)
                    colors = np.zeros((len(new_pcd.points), 3))
                    for i, distance in enumerate(distances):
                        normalized_distance = (distance - min_distance) / (max_distance - min_distance)
                        colors[i] = np.asarray([1 - normalized_distance, 0, normalized_distance])  # R-G-B

                    new_pcd.colors = o3d.utility.Vector3dVector(colors)

                    self.vis.add_geometry(new_pcd)
                    self.vis.update_geometry(new_pcd)
                    self.vis.reset_view_point(True)  # 重置视图点

                    self.nowpath = file_path
                    print("成功加载并显示点云文件。")

                else:
                    print("点云文件为空或无效，无法显示。")

            except Exception as e:
                print(f"加载点云文件时发生异常：{e}")
    ...
    def on_action12_triggered(self):
        try:
            self.pointnet_window = PointnetWindow()  # 实例化PointnetWindow类的对象
            self.pointnet_window.show()  # 显示PointnetWindow窗口
        except Exception as e:
